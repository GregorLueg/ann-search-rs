...
impl<T> IndexIo for IvfPqIndex<T>
where
    T: AnnSearchFloat,
{
    type Elem = T;

    const KIND: &'static str = "ivf_pq";
}
...
mod tests {
    ...
    fn test_query_expands_nprobe_when_probed_cells_underfill_k() {
        // 6 vectors, nlist=3 -> avg cell size 2. With nprobe=1 and k=4 the
        // top cell alone cannot cover k, so without auto-expansion the query
        // would short-return. Verify we still get 4 neighbours.
        let data = create_simple_dataset();
        let index = IvfPqIndex::build(
            data.as_ref(),
            Some(3),
        ...
        assert_eq!(indices.len(), 4, "expected auto-expanded nprobe to reach k");
