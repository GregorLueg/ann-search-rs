...
use crate::binary::{
    ...
    hnsw_rabitq::*, ivf_tq::*, qg::*, rotator::RotatorKind,
};
#[cfg(feature = "gpu")]
...
#[cfg(feature = "binary")]
/// Helper function to build an HNSW index over per-vertex RaBitQ codes
///
/// The low-memory sibling of [`build_qg_index`]: one code per vertex against
/// its cluster centroid instead of one per in-edge, so the codes cost
/// `n * padded_dim / 8` bytes however dense the graph is. Neighbours are
/// screened on the estimate and the vertices actually expanded get an exact
/// distance, so the returned distances are exact.
///
/// ### Params
///
/// * `mat` - Data as samples x features. See [`AnnMatrix`].
...
/// * `nlist` - Centroids the codes are taken against. `None` picks `sqrt(n)`
/// * `k_means_params` - Optional k-means settings, see [`KMeansTrainingParams`]
/// * `dist_metric` - Distance metric, `"euclidean"`/`"l2"` or `"cosine"`
/// * `seed` - Random seed, for reproducibility
/// * `verbose` - Print build progress
///
/// ### Returns
///
/// The [`HnswRaBitQIndex`], or an error on an unsupported metric
#[allow(clippy::too_many_arguments)]
...
#[cfg(feature = "binary")]
/// Helper function to build an HNSW-RaBitQ index with a named rotation
///
/// As [`build_hnsw_rabitq_index`], but the rotation is named rather than
/// resolved from the dimensionality.
///
/// ### Params
///
/// * `mat` - Data as samples x features. See [`AnnMatrix`].
...
/// The [`HnswRaBitQIndex`], or an error on an unsupported metric
#[allow(clippy::too_many_arguments)]
pub fn build_hnsw_rabitq_index_with_rotator<T>(
    mat: impl AnnMatrix<T>,
    m: usize,
    ef_construction: usize,
    nlist: Option<usize>,
...
#[cfg(feature = "binary")]
/// Helper function to query an HNSW-RaBitQ index
///
/// ### Params
///
/// * `query_mat` - Query data as samples x features. See [`AnnMatrix`].
...
#[cfg(feature = "binary")]
/// Helper function to generate the full kNN graph from an HNSW-RaBitQ index
///
/// ### Params
///
/// * `index` - Reference to the built HNSW-RaBitQ index
