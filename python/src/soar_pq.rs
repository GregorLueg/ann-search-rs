//! SOAR-PQ handle.
//!
//! IVF-PQ with spilling: every point also lands in a second cell, chosen by a
//! rule that accounts for the residual it already has in its primary. Better
//! recall per `nprobe` than plain IVF-PQ, for roughly twice the posting-list
//! size, which quantisation is what makes affordable.

use ann_search_rs::prelude::DimensionValidation;
use ann_search_rs::quantised::soar_pq::SoarPqIndex;
use ann_search_rs::{build_soar_pq_index, query_soar_pq_index, query_soar_pq_index_self};
...
use crate::soar::soar_rule;

ann_handle!(PySoarPq, SoarPqInner, SoarPqIndex, "SoarPq", method, {
    /// Cluster with spilling, then encode each residual.
    ///
    /// ### Params
    ///
    ...
    ///   heuristic.
    /// * `rule` - Spilling rule, or `None` to let the library pick per
    ///   metric.
    ...
    #[staticmethod]
    #[pyo3(signature = (
            x, *, m, metric, nlist = None, rule = None, rule_param = None, kmeans_iters = None,
            kmeans_balanced = false, n_pq_centroids = None, seed = 42, verbose = false
        ))]
    ...
    fn build(
        ...
        build_dispatch!(py, x, SoarPqInner, |data, n, dim| build_soar_pq_index(
            (data, n, dim),
            nlist,
            m,
            rule,
    ...
    ///   The main recall knob. A spilled index scans about twice the
    ///   candidates at a fixed `nprobe`, so read the trade against query
    ///   time rather than against `nprobe`.
    ...
    fn query<'py>(
        ...
        match &self.inner {
            SoarPqInner::F32(idx) => query_arm!(py, q, k, f32, "float32", |data, n, dim| {
                query_soar_pq_index((data, n, dim), idx, k, nprobe, return_distance, verbose)
            }),
            SoarPqInner::F64(idx) => query_arm!(py, q, k, f64, "float64", |data, n, dim| {
                query_soar_pq_index((data, n, dim), idx, k, nprobe, return_distance, verbose)
            }),
        }
    }
    ...
    fn query_self<'py>(
        ...
        match &self.inner {
            SoarPqInner::F32(idx) => {
                self_arm!(py, k, || query_soar_pq_index_self(
                    idx,
                    k,
                    nprobe,
                    return_distance,
