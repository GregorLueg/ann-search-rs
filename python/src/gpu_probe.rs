...
//!   the Vulkan loader.
//!
//! [`gpu_available`] answers both at once, since a caller choosing between a
//! CPU and a GPU estimator has no use for the distinction.
...
fn probe() -> bool {
    ...
    let ok = std::panic::catch_unwind(|| {
        let device = crate::gpu_handle::default_device();
        let _ = <crate::gpu_handle::Rt as Runtime>::client(&device);
    })
    .is_ok();
