...
use ann_search_rs::{build_soar_opq_index, query_soar_opq_index, query_soar_opq_index_self};
...
ann_handle!(PySoarOpq, SoarOpqInner, SoarOpqIndex, "SoarOpq", method, {
    /// Cluster with spilling, learn the rotation, then encode.
    ///
    /// ### Params
    ///
    ...
    #[pyo3(signature = (
            ...
            kmeans_balanced = false, n_pq_centroids = None, opq_iters = None, seed = 42,
            verbose = false
        ))]
    ...
    fn build(
        ...
        build_dispatch!(py, x, SoarOpqInner, |data, n, dim| build_soar_opq_index(
            (data, n, dim),
            nlist,
            m,
            rule,
    ...
    fn query<'py>(
        ...
        match &self.inner {
            SoarOpqInner::F32(idx) => query_arm!(py, q, k, f32, "float32", |data, n, dim| {
                query_soar_opq_index((data, n, dim), idx, k, nprobe, return_distance, verbose)
            }),
            SoarOpqInner::F64(idx) => query_arm!(py, q, k, f64, "float64", |data, n, dim| {
                query_soar_opq_index((data, n, dim), idx, k, nprobe, return_distance, verbose)
            }),
        }
    }
    ...
    fn query_self<'py>(
        ...
        match &self.inner {
            SoarOpqInner::F32(idx) => {
                self_arm!(py, k, || query_soar_opq_index_self(
                    idx,
                    k,
                    nprobe,
                    return_distance,
