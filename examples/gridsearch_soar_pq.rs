//! Gridsearch for SOAR-PQ against plain IVF-PQ.
//!
...
//! that matters is against an IVF-PQ index with **twice the subspaces**, which
//! is the third column here. Beating IVF-PQ at the same `m` while using twice
...
//!
//!   cargo run --example gridsearch_soar_pq --release --features quantised -- \
...
/// exact search. Measured on 50k x 512 cell embeddings they still do not: all
/// four land within ~0.005 recall and converge to an identical plateau. Kept as
/// a check that the choice still does not matter on your data, since one
/// synthetic dataset is thin evidence for dropping the knob.
...
fn main() {
    ...
    println!(
        "PQ subspaces: m={} (SOAR-PQ and IVF-PQ), m={} (IVF-PQ at equal memory)",
        m, m2
    );
    println!("-----------------------------");
    ...
    ///////////////////////////////////////////////
    // Sweep A: SOAR-PQ vs IVF-PQ at m and at 2m //
    ///////////////////////////////////////////////

    let mut results_a = vec![baseline_row()];

    println!("-----------------------------");
    println!("Sweep A: IVF-PQ(m), IVF-PQ(2m), SOAR-PQ(m) across nlist");
    println!("-----------------------------");
    ...
    for &nlist in &nlist_values {
        println!("Building IVF-PQ (nlist={}, m={})...", nlist, m);
        let start = Instant::now();
        let pq_idx = build_ivf_pq_index(
            data.as_ref(),
            Some(nlist),
            m,
            None,
        ...
        println!("Building IVF-PQ (nlist={}, m={})...", nlist, m2);
        let start = Instant::now();
        let pq2_idx = build_ivf_pq_index(
            data.as_ref(),
            Some(nlist),
            m2,
            None,
        ...
        println!("Building SOAR-PQ (nlist={}, m={})...", nlist, m);
        let start = Instant::now();
        let soar_idx = build_soar_pq_index(
            data.as_ref(),
            Some(nlist),
            m,
            None,
        ...
        for nprobe in nprobe_values(nlist) {
            ...
            results_a.push(BenchmarkResultSize {
                method: format!("IVFPQ-m{}-nl{}-np{}", m, nlist, nprobe),
            ...
            let q_ms = start.elapsed().as_secs_f64() * 1000.0;
            results_a.push(BenchmarkResultSize {
                method: format!("IVFPQ-m{}-nl{}-np{}", m2, nlist, nprobe),
            ...
            let q_ms = start.elapsed().as_secs_f64() * 1000.0;
            results_a.push(BenchmarkResultSize {
                method: format!("SOARPQ-{}-m{}-nl{}-np{}", soar_tag, m, nlist, nprobe),
    ...
    print_results_size(
        &format!(
            "Sweep A: SOAR-PQ vs IVF-PQ, {}k samples, {}D",
            cli.n_samples / 1000,
            cli.dim
        ),
        &results_a,
    ...
    for rule in RULES {
        ...
        println!("Building SOAR-PQ (rule={}, nlist={})...", tag, nlist);
        let start = Instant::now();
        let idx = build_soar_pq_index(
            data.as_ref(),
            Some(nlist),
            m,
            Some(rule),
        ...
        for nprobe in nprobe_values(nlist) {
            ...
            results_b.push(BenchmarkResultSize {
                method: format!("SOARPQ-{}-np{}", tag, nprobe),
