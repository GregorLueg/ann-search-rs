...
fn main() {
    ...
    let start = Instant::now();
    let exhaustive_idx_bf16 =
        build_exhaustive_bf16_index(data.as_ref(), &cli.distance, false).unwrap();
    let build_time = start.elapsed().as_secs_f64() * 1000.0;
    ...
    let start = Instant::now();
    let (approx_neighbors, _) = query_exhaustive_bf16_index(
        query_data.as_ref(),
        &exhaustive_idx_bf16,
        cli.k,
        false,
    ...
    let (approx_neighbors_self, _) =
        query_exhaustive_bf16_self(&exhaustive_idx_bf16, cli.k, false, false).unwrap();
    let self_query_time = start.elapsed().as_secs_f64() * 1000.0;
    ...
    for nlist in nlist_values {
        ...
        for nprobe in &nprobe_values {
            ...
            let start = Instant::now();
            let (approx_neighbors, _) = query_ivf_bf16_index(
                query_data.as_ref(),
                &ivf_bf16_idx,
                cli.k,
                Some(*nprobe),
        ...
        let start = Instant::now();
        let (approx_neighbors_self, _) =
            query_ivf_bf16_self(&ivf_bf16_idx, cli.k, Some(nprobe_self), false, false).unwrap();
        let self_query_time = start.elapsed().as_secs_f64() * 1000.0;
