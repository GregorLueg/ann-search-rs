...
run_gpu_benchmarks() {
    ...
    # run_common_patterns "cargo run --example gridsearch_carga --release --features gpu" "GPU (Cagra)"
    ...
    for n_cells in 250000 500000; do
        for n_dim in 64 128; do
            ...
            cargo run --example gridsearch_carga --release --features gpu -- --distance euclidean --n-cells ${n_cells} --dim ${n_dim} --data lowrank
        done
    done
    ...
    for n_cells in 250000 500000 1000000; do
        for n_dim in 32 64; do
            cargo run --example knn_comparison_carga --release --features gpu -- --distance euclidean --data lowrank --n-cells ${n_cells} --dim ${n_dim}
        done
    done
}
